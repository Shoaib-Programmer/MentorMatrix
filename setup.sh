...
create_env_file() {
  if [ ! -f .env ]; then
    ...
    while [ -z "$GOOGLE_CLIENT_ID" ]; do
      read -p "GOOGLE_CLIENT_ID (cannot be empty): " GOOGLE_CLIENT_ID
    done

    while [ -z "$GOOGLE_CLIENT_SECRET" ]; do
      read -p "GOOGLE_CLIENT_SECRET (cannot be empty): " GOOGLE_CLIENT_SECRET
    done

    while [ -z "$GOOGLE_DISCOVERY_URL" ]; do
      read -p "GOOGLE_DISCOVERY_URL (cannot be empty): " GOOGLE_DISCOVERY_URL
    done
    ...
    echo "# Google API credentials" >> .env
